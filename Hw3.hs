...
evalOp Divide (IntVal i) (IntVal j) = if j == 0
                                      then do evalS (Print "Error: Division by zero: " (Val (IntVal j)))
                                              throwError (IntVal 1)
                                      else return (IntVal (i `div` j))
...
evalE (Var x)      = do s <- get
                        case Map.lookup x s of
                          Just v -> return v
                          Nothing -> do evalS (Print ("Error: Reading an undefined variable: " ++ show x ++ " ") (Val (IntVal 0)))
                                        throwError (IntVal 0)
evalE (Val v)      = return v
evalE (Op o e1 e2) = do i <- evalE e1
                        j <- evalE e2
                        case (i, j) of
                          (IntVal i', IntVal j') -> evalOp o i j
                          (IntVal i', _)         -> do evalS (Print ("Error: Binary operation " ++ show o ++ " on IntVal and Non-IntVal: ") e2)
                                                       throwError (IntVal 2)
                          (_, IntVal j')         -> do evalS (Print ("Error: Binary operation " ++ show o ++ " on IntVal and Non-IntVal: ") e1)
                                                       throwError (IntVal 2)
                          (_, _)                 -> do evalS (Print ("Error: Binary operation " ++ show o ++ " on Non-IntVal and Non-IntVal: " ++ show i ++ ", ") e2)
                                                       throwError (IntVal 2)

evalS :: (MonadState Store m, MonadError Value m, MonadWriter String m) => Statement -> m ()
evalS (Assign x e)     = do a <- evalE e
                            s <- get
                            put (Map.insert x a s)
evalS (If e s1 s2)     = do c <- evalE e
                            case c of
                              BoolVal True  -> evalS s1
                              BoolVal False -> evalS s2
                              otherwise     -> do evalS (Print "Error: Non-BoolVal in IF condition: " e)
                                                  evalS (Throw (Val (IntVal 2)))
evalS w@(While e s)    = do c <- evalE e
                            case c of
                              BoolVal True  -> evalS (Sequence s w)
                              BoolVal False -> return ()
                              otherwise     -> do evalS (Print "Error: Non-BoolVal in WHILE condition: " e)
                                                  evalS (Throw (Val (IntVal 2)))
evalS (Sequence s1 s2) = evalS s1 >> evalS s2
evalS Skip             = return ()
evalS (Print str e)    = do v <- evalE e
                            tell (str ++ show v ++ "\n")
evalS (Throw e)        = do v <- evalE e
                            throwError v
evalS (Try s x h)      = let handler err = do m <- get
                                              put (Map.insert x err m)
                                              evalS h
                         in catchError (evalS s) handler
...
runEval act = runState (runWriterT (runErrorT act))
...
bstBalance :: BST k v -> BST k v
bstBalance Emp = Emp
bstBalance (Bind k v l r)
  | height l - height r > 1 =
    case l of
      (Bind _ _ l' r') -> if height l' < height r'
                          then bstRotR (Bind k v (bstRotL l) r)
                          else bstRotR (Bind k v l r)
      otherwise        -> bstRotR (Bind k v l r)
  | height r - height l > 1 =
    case r of
      (Bind _ _ l' r') -> if height l' > height r'
                          then bstRotL (Bind k v l (bstRotR r))
                          else bstRotL (Bind k v l r)
      otherwise        -> bstRotL (Bind k v l r)
  | otherwise = Bind k v l r

bstRotR :: BST k v -> BST k v
bstRotR Emp                             = Emp
bstRotR (Bind k v Emp r)                = Bind k v Emp r
bstRotR (Bind k v (Bind k' v' l' r') r) = Bind k' v' l' (Bind k v r' r)

bstRotL :: BST k v -> BST k v
bstRotL Emp                             = Emp
bstRotL (Bind k v l Emp)                = Bind k v l Emp
bstRotL (Bind k v l (Bind k' v' l' r')) = Bind k' v' (Bind k v l l') r'
...
bstInsert k' v' (Bind k v l r)
  | k' == k = Bind k v' l r
  | k' < k  = bstBalance (Bind k v (bstInsert k' v' l) r)
  | k' > k  = bstBalance (Bind k v l (bstInsert k' v' r))
...
bstDelete k' (Bind k v l r)
  | k' > k    = bstBalance (Bind k v l (bstDelete k' r))
  | k' < k    = bstBalance (Bind k v (bstDelete k' l) r)
  | otherwise = bstBalance (case (l, r) of
                              (Emp, r) -> r
                              (l, Emp) -> l
                              (l, r)   -> Bind k'' v'' l (bstDelete k'' r)
...
prop_bitSubtractor_Correct cin xs =
  case (binary xs) of
    0 -> binary (sampleN out) == 0 && binary ([sample1 cout]) == binary (sample1 cin)
    otherwise -> binary (sampleN out ++ [sample1 cout]) == binary xs - binary (sample1 cin)
...
halfsub (x,y) = (diff,cout)
  where diff  = xor2 (x, y)
        notx  = xor2 (x, high)
        cout  = and2 (notx, y)

bitSubtractorDetail :: (Signal, [Signal]) -> ([Signal], Signal)
bitSubtractorDetail (cin, [])   = ([], cin)
bitSubtractorDetail (cin, x:xs) = (diff:diffs, cout)
  where (diff, c)               = halfsub (x, cin)
        (diffs, cout)           = bitSubtractor (c, xs)

bitSubtractor :: (Signal, [Signal]) -> ([Signal], Signal)
bitSubtractor (cin, xs) = (dem, cout)
  where (diffs, cout)   = bitSubtractorDetail (cin, xs)
        (_, dem)        = demuxN (diffs, cout)
...
test6 = probe [("cin",cin), ("in1",in1), ("in2",in2), ("in3",in3), ("in4",in4),
               (" s1",s1), ("s2",s2), ("s3",s3), ("s4",s4), (" c",c)]
  where
    cin = high
    in1 = low
...
test7 = probe [("cin",cin), ("in1",in1), ("in2",in2), ("in3",in3), ("in4",in4),
               (" s1",s1), ("s2",s2), ("s3",s3), ("s4",s4), (" c",c)]
  where
    cin = high
    in1 = low
...
multiplier ([], _) = []
multiplier (_, []) = []
multiplier (xs, ys@(y:y')) = take len (adder (dem, [low] ++ multiplier (xs, y')))
  where (dem, _) = demuxN (xs, y)
...
test9 = probe [ ("x1", x1), ("x2",x2), 
                (" y1",y1), ("y2",y2), 
                (" s1",s1), ("s2",s2), ("s3",s3), ("s4",s4)]
  where xs@[x1,x2] = [high,high]
        ys@[y1,y2] = [high,high]
        [s1,s2,s3,s4] = multiplier (xs, ys)

test10 = probe [ ("x1", x1), ("x2",x2), 
                (" y1",y1), ("y2",y2), 
                (" s1",s1), ("s2",s2), ("s3",s3), ("s4",s4)]
  where xs@[x1,x2] = [low,high]
        ys@[y1,y2] = [low,high]
        [s1,s2,s3,s4] = multiplier (xs, ys)

test11 = probe [ ("x1", x1), ("x2",x2), ("x3",x3),
                (" y1",y1), ("y2",y2), ("y3",y3),
...
test12 = probe [ ("x1", x1), ("x2",x2), ("x3",x3),
                (" y1",y1), ("y2",y2), ("y3",y3),
                (" s1",s1), ("s2",s2), ("s3",s3), ("s4",s4), ("s5",s5), ("s6",s6) ]
  where xs@[x1,x2,x3] = [high,low,high]
        ys@[y1,y2,y3] = [high,low,high]
        [s1,s2,s3,s4,s5,s6] = multiplier (xs, ys)
